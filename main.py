...
parser.add_argument(
    "-f", "--force-update", action="store_true",
    help="以前ダウンロードしたコンテンツも上書きして再ダウンロードします。")
parser.add_argument(
    "-P", "--update-posts", action="store_true",
    help="ダウンロードしたことのある投稿データを再ダウンロードします。前回のファイルを上書きせずに別のファイルとして保存されます。")
# parser.add_argument(
#     "-b", "--before-id", type=int, help="指定した投稿ID以前（その投稿も含む）の投稿をダウンロードします。")
parser.add_argument(
    "-l", "--page-limit", type=int,
    help="1投稿者あたりの取得ページ数。省略した場合は可能な限り取得します。")
# parser.add_argument(
#     "--ignore-free-posts", action="store_true",
#     help="無料の投稿に含まれる画像はダウンロードしません。")
# parser.add_argument(
#     "--ignore-adult-contents", action="store_true",
#     help="成人向けの投稿に含まれる画像はダウンロードしません。")
